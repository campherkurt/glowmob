...
STATUS_CHOICES = (
    (STATUS_MODERATION, 'Awating Moderation'),
...
#        )
        return super(ThreadedCommentManager, self).get_query_set().exclude(status=STATUS_MODERATION)
...
class ThreadedComment(models.Model):
    ...
    cellphone = models.CharField(_("cell number"), max_length=255, blank=False, null=True)
    ...
    status = models.IntegerField(_('Status'), choices=STATUS_CHOICES, default=STATUS_MODERATION)
    # Extra Field
    ip_address = models.IPAddressField(_('IP address'),max_length=30, null=True, blank=True)
    ...
    def get_content_object_str(self):
        ...
        if self.content_type == story:
            ...
            return self.content_object.story.title + " - " + self.content_object.title
