...
from apps.basic_profiles.models import REGIONS, USER_TYPE_MOBI
...
class SignupForm(GroupForm):
    ...
    username = forms.CharField(
        label = _("* Choose a username(no spaces, 4-16 chars)"),
        max_length = 30,
    ...
    mobile = forms.CharField(label=_("Mobile Number"), required = False, widget=forms.TextInput())
    region = forms.ChoiceField(label=_("Region"), choices=REGIONS, required = True)
    ...
    terms = forms.BooleanField(label=_('I agree to the Yoza Terms of Use'), required=True)
    ...
    def clean_username(self):
        ...
        if len(self.cleaned_data["username"]) < USERNAME_MIN or len(self.cleaned_data["username"]) > USERNAME_MAX:
            raise forms.ValidationError(_("Usernames must be between 4 - 16 characters."))
        try:
            user = User.objects.filter(username__iexact=self.cleaned_data["username"])[0]
        except:
    ...
    def clean_password2(self):
        if "password1" in self.cleaned_data and "password2" in self.cleaned_data:
            if self.cleaned_data["password1"] != self.cleaned_data["password2"]:
                self._errors["password1"] = ErrorList(["You must type the same password each time."])
...
#                raise forms.ValidationError(_("You must type the same password each time."))
    ...
    def save(self, request=None):
        ...
        if confirmed:
            ...
            if email:
                ...
                try:
                    ...
                    print "Error adding email email"
        ...
        profile.user_type = USER_TYPE_MOBI
        profile.save()
