...
from apps.story.forms import CommentForm
from django.contrib import messages
from apps.story.models import Story, Chapter
from settings import MXIT_HEADER
...
def _preview(request, context_processors, extra_context, form_class=ThreadedCommentForm):
    ...
    return HttpResponseRedirect(request.POST["prev"])
    ...
    if form.is_valid():
        ...
        if not edit_id:
            ...
            if request.META.has_key(MXIT_HEADER):
                new_comment.status = threadedcomments.models.STATUS_MODERATION
            else:
                new_comment.status = threadedcomments.models.STATUS_APPROVED
        ...
        if model == ThreadedComment:
            if add_messages:
                if not request.META.has_key(MXIT_HEADER):
                    messages.add_message(request, messages.SUCCESS, "Your message has been posted successfully.")
                else:
                    messages.add_message(request, messages.SUCCESS, "Your comment has been submitted and will be moderated shortly.")
        else:
...
<errorlist>
    ...
    {% endfor %}
        ...
        if content_type == story:
            story = Story.public.get(id=object_id)
            ...
            chapter = Chapter.objects.get(id=object_id)
            try:
                p = request.POST["p"]
            except:
                p = None
...
def comment(*args, **kwargs):
    ...
    kwargs['form_class'] = CommentForm
    kwargs['add_messages'] = True
...
def can_delete_comment(comment, user):
    ...
    if hasattr(comment, 'user') and comment.user == user:
        return True
    return False
#def comment_report(request, object_id):
#    tc = get_object_or_404(ThreadedComment, id=int(object_id))
#    tc.status = threadedcomments.models.STATUS_REPORTED
#    tc.save()
#    messages.add_message(request, messages.SUCCESS, "The comment has been reported.")
#    return HttpResponseRedirect(_get_next(request))
